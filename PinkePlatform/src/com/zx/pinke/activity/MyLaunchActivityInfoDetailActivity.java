...
public class MyLaunchActivityInfoDetailActivity extends BaseActivity {
	...
	private void initViews(){
		...
		mBeginDateTv.setText(getString(R.string.begin_time_at,mPublicService.getStartDate() == null ? "无" : mPublicService.getStartDate()));
		mEndDateTv.setText(getString(R.string.end_time_at,mPublicService.getEndDate() == null ? "无" : mPublicService.getEndDate()));
		mCategoryTv.setText(getString(R.string.category_at,mPublicService.getCategory()));
		mPlaceTv.setText(getString(R.string.place_at,mPublicService.getPlace()));
		if (mPublicService.getNeedNum()==0) {
			mJoinTv.setText(getString(R.string.join_number,mPublicService.getJoinNum()));
		}else {
			mJoinTv.setText(getString(R.string.join_and_last_number,mPublicService.getJoinNum(),mPublicService.getNeedNum()-mPublicService.getJoinNum()));
		}
		mDetailTv.setText(Html.fromHtml(mPublicService.getContent()==null?"暂无":mPublicService.getContent()));
		
		mContactsTv.setText(getString(R.string.contacts_of,mPublicService.getContacts()));
		mNeed.setText(getString(R.string.need_number, mPublicService.getNeedNum()==0?"不限":mPublicService.getNeedNum()));
		...
		mSeeJoin.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent=new Intent(MyLaunchActivityInfoDetailActivity.this, JoinUserListActivity.class);
				intent.putExtra("mid", mPublicService.getKey());
				startActivity(intent);
			}
		});
		
		if (mPublicService.getLaunch().equals(AccountInfo.getAccountInfo().getAccount())) {
			...
			for (AccountInfo user : mPublicService.getJoinUser()) {
				if (user.getAccount().equals(AccountInfo.getAccountInfo().getAccount())) {
					isJoin=true;
				}
			}
			if (isJoin) {
	...
	private class JoinListener implements View.OnClickListener{
		...
		public void onClick(View v) {
			...
			try {
				...
				mBackgroundQueryHandler.startPost(TOKEN_JOIN, params, uri, params,sessionId);
